...
function InvoiceDetail({ summary, payload, companyId, loading, close, print, download, cancel, cancelBusy, openRelated, relatedSalesIds }: { summary: InvoiceSummary; payload: DetailPayload | null; companyId: string; loading: boolean; close: () => void; print: () => void; download: () => void; cancel: (reason: string) => void; cancelBusy: boolean; openRelated: (salesId: string) => void; relatedSalesIds: Set<string> }) {
  ...
  return <div className="fixed inset-0 z-[75] overflow-y-auto bg-slate-950/65 p-4"><article className="relative mx-auto my-5 max-w-5xl rounded-3xl bg-white p-6 shadow-2xl">
    ...
    <div className="flex items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-black uppercase tracking-wider text-emerald-700">Invoice</p><span className={`rounded-full px-2.5 py-1 text-[11px] font-black uppercase ${canceled ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'}`}>{canceled ? 'Dibatalkan' : 'Aktif'}</span></div><h2 className="mt-2 text-2xl font-black">{summary.invoiceNo}</h2><p className="mt-1 text-sm text-slate-500">{summary.customerName} · {summary.storeName}</p><p className={`mt-1 text-sm ${summary.invoiceDate ? 'text-slate-500' : 'text-amber-700'}`}>{summary.invoiceDate ? `Tanggal Invoice ${invoiceDateLabel(summary.invoiceDate)}` : `Waktu konfirmasi ${dateTime(summary.postedAt)}`}</p><a href={retailOrderHref(companyId, summary.salesId)} className="mt-2 inline-flex items-center gap-1 text-sm font-black text-emerald-700 underline decoration-emerald-200 underline-offset-4 hover:text-emerald-900">Buka order sumber Retail<ArrowRight className="h-4 w-4"/></a></div><div className="flex shrink-0 items-center gap-2">{activity && <button type="button" onClick={() => setHistoryOpen(true)} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 px-3 text-sm font-bold text-slate-700 hover:bg-slate-50"><History className="h-4 w-4"/><span className="hidden sm:inline">Riwayat</span></button>}<button onClick={close} className="rounded-xl bg-slate-100 p-2" aria-label="Tutup"><X className="h-5 w-5"/></button></div></div>
