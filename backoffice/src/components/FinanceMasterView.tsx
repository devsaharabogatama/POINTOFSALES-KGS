...
function RuleEditor({ session, categories, events, functions, accounts, close, complete }: { session: Session; categories: Category[]; events: SystemEvent[]; functions: AccountFunction[]; accounts: Account[]; close: () => void; complete: () => Promise<void> }) {
  ...
  async function submit(event: React.FormEvent) { event.preventDefault(); setSaving(true); setError(''); try { const response = await fetch('/api/master/finance-masters', { method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders(session) }, body: JSON.stringify({ entityType: 'RULE', ...form, effectiveFrom: instantValue(form.effectiveFrom), effectiveTo: form.effectiveTo ? instantValue(form.effectiveTo) : null }) }); const payload = await response.json() as { error?: string }; if (!response.ok) throw new Error(friendlyError(payload.error)); await complete() } catch (caught) { setError(caught instanceof Error ? caught.message : 'Gagal menyimpan mapping.') } finally { setSaving(false) } }
...
function FallbackEditor({ session, functions, accounts, close, complete }: { session: Session; functions: AccountFunction[]; accounts: Account[]; close: () => void; complete: () => Promise<void> }) {
  ...
  const availableAccounts = accounts.filter((item) => item.is_active && item.is_postable && (!selectedFunction || selectedFunction.compatible_account_types.includes(item.account_type)))
  function changeFunction(accountFunctionKey: string) { setForm({ ...form, accountFunctionKey, accountId: '' }) }
  async function submit(event: React.FormEvent) { event.preventDefault(); setSaving(true); setError(''); try { const response = await fetch('/api/master/finance-masters', { method: 'POST', headers: { 'Content-Type': 'application/json', ...authHeaders(session) }, body: JSON.stringify({ entityType: 'FALLBACK', ...form, effectiveFrom: instantValue(form.effectiveFrom), effectiveTo: form.effectiveTo ? instantValue(form.effectiveTo) : null }) }); const payload = await response.json() as { error?: string }; if (!response.ok) throw new Error(friendlyError(payload.error)); await complete() } catch (caught) { setError(caught instanceof Error ? caught.message : 'Gagal menyimpan fallback.') } finally { setSaving(false) } }
