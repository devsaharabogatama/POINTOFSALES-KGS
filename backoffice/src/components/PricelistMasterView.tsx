...
function RuleValueHint({ rule, basePrice }: { rule: RuleForm; basePrice: number | string | null }) {
  const normalPrice = Number(basePrice)
  const value = Number(rule.value)
  const valueIsValid = rule.value.trim() !== '' && Number.isFinite(value) && value >= 0
    && (rule.pricingMethod !== 'DISCOUNT_PERCENT' || value <= 100)
  const priceIsValid = Number.isFinite(normalPrice) && normalPrice >= 0
  const finalPrice = !valueIsValid || !priceIsValid
    ? null
    : rule.pricingMethod === 'FIXED_PRICE'
      ? value
      : rule.pricingMethod === 'DISCOUNT_AMOUNT'
        ? Math.max(normalPrice - value, 0)
        : Math.max(normalPrice * (100 - value) / 100, 0)
  const help = rule.pricingMethod === 'FIXED_PRICE'
    ? `Contoh: dari ${rupiah(basePrice)} menjadi Rp4.000, isi 4000.`
    : rule.pricingMethod === 'DISCOUNT_AMOUNT'
      ? 'Potongan nominal dikenakan pada setiap UOM di tier ini.'
      : 'Contoh: isi 5 untuk memberi diskon 5% dari harga normal UOM.'

  return <div className="mt-1 space-y-1 text-xs">
    <span className="block text-slate-400">{help}</span>
    {finalPrice !== null && <span className="block font-bold text-emerald-700">Perkiraan harga akhir: {rupiah(finalPrice)}</span>}
  </div>
}
