      ...
      >
        <div className="flex items-center gap-2 border-b border-slate-200 p-3">
          ...
          <input
            ...
            aria-label="Ketik untuk mencari pilihan"
            className="min-w-0 flex-1 bg-transparent px-1 py-2 text-sm outline-none"
          />
          {query && (
            <button type="button" aria-label="Hapus pencarian" onClick={() => setQuery("")} className="rounded-lg p-1 text-slate-500 hover:bg-slate-100">
              <X className="h-4 w-4" />
            </button>
          )}
        </div>
          ...
          {filtered.map((option, index) => {
            ...
            return (
              <button
                ...
                className={`flex w-full items-center justify-between gap-3 rounded-xl px-3 py-2.5 text-left text-sm ${index === focusedIndex ? "bg-emerald-50 text-emerald-900" : "hover:bg-slate-50"} disabled:cursor-not-allowed disabled:opacity-40`}
              >
        ...
        <div className="flex items-center justify-between border-t border-slate-100 px-3 py-2 text-[11px] text-slate-400">
          <span>{filtered.length} pilihan</span>
          <span className="inline-flex items-center gap-1">Enter untuk memilih <ChevronDown className="h-3 w-3" /></span>
        </div>
      </section>
    </>,
    document.body,
