...
export function BackofficeSalesOrderView({ session, companyId, companyName, canCreate, canEdit, canManage, notify, openProcessSettings, openSalesReturn, openRetainedRetailReturn }: { session: Session; companyId: string; companyName: string; canCreate: boolean; canEdit: boolean; canManage: boolean; notify: (message: string) => void; openProcessSettings: () => void; openSalesReturn: (salesOrderId: string, returnId?: string) => void; openRetainedRetailReturn: (retailSalesId: string) => void }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[1120px] table-fixed text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="w-[19%] px-5 py-4">Nomor</th><th className="w-[18%] px-5 py-4">Customer</th><th className="w-[19%] px-5 py-4">Tanggal Order</th><th className="w-[15%] px-5 py-4">Status Pengiriman</th>{documentKind === "SALES_ORDER" && <th className="w-[16%] px-5 py-4">Status Invoice</th>}<th className="w-[13%] px-5 py-4 text-right">Total</th></tr></thead><tbody className="divide-y divide-slate-100">
      ...
      {visibleHistory.map((row) => <tr key={`retail-${row.id}`} onClick={() => void openHistory(row)} className="cursor-pointer bg-amber-50/30 hover:bg-amber-50"><td className="px-5 py-4"><div className="font-black">{row.documentNo}</div><div className="mt-1 flex flex-wrap items-center gap-2"><span className="rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-black text-amber-900">Asal Retail</span><span className="text-xs text-slate-500">Histori sebelum mode Backoffice</span></div></td><td className="px-5 py-4"><div className="font-bold">{row.customerName ?? '-'}</div><div className="text-xs text-slate-500">{row.storeName ?? '-'}</div></td><td className="px-5 py-4">{dateText(row.orderDate)}{row.dueDate && <div className="text-xs text-slate-500">Jatuh tempo {dateText(row.dueDate)}</div>}</td><td className="px-5 py-4"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold">{retailStatusLabel(row)}</span></td>{documentKind === 'SALES_ORDER' && <td className="px-5 py-4">{row.invoiceSnapshotId ? <button onClick={(event) => { event.stopPropagation(); void openHistory(row); }} className="text-xs font-bold text-emerald-700">Invoice asli</button> : <span className="text-xs text-slate-500">Belum ada Invoice</span>}</td>}<td className="px-5 py-4 text-right font-black">{money(row.total)}</td></tr>)}
