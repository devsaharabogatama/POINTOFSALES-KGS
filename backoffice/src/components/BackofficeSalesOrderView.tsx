...
import { netCommercialAmount, type SalesReturnCommercialAdjustment } from '@/lib/sales-return-commercial';
...
export function BackofficeSalesOrderView({ session, companyId, companyName, canCreate, canEdit, canManage, notify, openProcessSettings, openSalesReturn, openRetainedRetailReturn }: { session: Session; companyId: string; companyName: string; canCreate: boolean; canEdit: boolean; canManage: boolean; notify: (message: string) => void; openProcessSettings: () => void; openSalesReturn: (salesOrderId: string, returnId?: string) => void; openRetainedRetailReturn: (retailSalesId: string) => void }) {
  ...
  if (selectedHistory?.companyId === companyId) return <OfficeRetailHistoryDetail key={selectedHistory.id} row={selectedHistory} session={session} companyName={companyName} notify={notify} back={() => setSelectedHistory(null)} openReturn={openRetainedRetailReturn} />;
  ...
  return <div className="space-y-5">
    ...
    <section className="overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="w-full min-w-[1120px] table-fixed text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr><th className="w-[19%] px-5 py-4">Nomor</th><th className="w-[18%] px-5 py-4">Customer</th><th className="w-[19%] px-5 py-4">Tanggal Order</th><th className="w-[15%] px-5 py-4">Status Pengiriman</th>{documentKind === "SALES_ORDER" && <th className="w-[16%] px-5 py-4">Status Invoice</th>}<th className="w-[13%] px-5 py-4 text-right">Total</th></tr></thead><tbody className="divide-y divide-slate-100">
      ...
      {orders.map((order) => { const credited=Number(order.returnAdjustment?.creditedAmount??0); return <tr key={order.id} onClick={() => setSelected(order)} className="cursor-pointer hover:bg-slate-50"><td className="px-5 py-4"><div className="font-black">{order.orderNo ?? order.quotationNo}</div><div className="mt-1 flex flex-wrap items-center gap-2"><span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-black text-emerald-800">Backoffice</span>{order.orderNo && <span className="text-xs text-slate-500">Asal {order.quotationNo}</span>}</div></td><td className="px-5 py-4"><div className="font-bold">{order.customerSnapshot.name ?? "-"}</div><div className="text-xs text-slate-500">{order.customerSnapshot.code ?? "-"}</div></td><td className="px-5 py-4"><div>{dateText(order.orderDate)}</div><div className="text-xs text-slate-500">Rencana kirim {dateText(order.plannedDeliveryDate)}</div>{order.isTempo && <div className="text-xs text-slate-500">Jatuh tempo {dateText(order.dueDate)}</div>}</td><td className="px-5 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${fulfillmentClass[order.fulfillmentStatus]}`}>{order.orderNo ? fulfillmentLabel[order.fulfillmentStatus] : order.status === "CANCELED" ? "Dibatalkan" : "Draft"}</span></td>{documentKind === "SALES_ORDER" && <td className="px-5 py-4"><button type="button" disabled={order.invoiceStatus === "NOT_READY"} onClick={(event) => { event.stopPropagation(); if (order.invoiceStatus === "DRAFT" && order.draftInvoiceId) setInvoiceMode({ invoiceId: order.draftInvoiceId }); else if (order.invoiceStatus === "READY") setInvoiceMode({ salesOrderId: order.id }); else if (order.invoiceStatus === "PARTIALLY_INVOICED" || order.invoiceStatus === "INVOICED") setInvoiceMode({ salesOrderId: order.id, list: true }); }} className={`rounded-full px-2.5 py-1 text-xs font-bold ${invoiceClass[order.invoiceStatus]} disabled:cursor-default`}>{invoiceLabel[order.invoiceStatus]}</button></td>}<td className="px-5 py-4 text-right"><strong className="block">{money(netCommercialAmount(Number(order.grandTotal),credited))}</strong>{credited>0&&<span className="text-xs text-slate-500">Awal {money(Number(order.grandTotal))}</span>}</td></tr>})}
      {visibleHistory.map((row) => { const credited=Number(row.returnAdjustment?.creditedAmount??row.creditedAmount??0); return <tr key={`retail-${row.id}`} onClick={() => void openHistory(row)} className="cursor-pointer bg-amber-50/30 hover:bg-amber-50"><td className="px-5 py-4"><div className="font-black">{row.documentNo}</div><div className="mt-1 flex flex-wrap items-center gap-2"><span className="rounded-full bg-amber-100 px-2 py-0.5 text-[11px] font-black text-amber-900">Asal Retail</span><span className="text-xs text-slate-500">Histori sebelum mode Backoffice</span></div></td><td className="px-5 py-4"><div className="font-bold">{row.customerName ?? '-'}</div><div className="text-xs text-slate-500">{row.storeName ?? '-'}</div></td><td className="px-5 py-4">{dateText(row.orderDate)}{row.dueDate && <div className="text-xs text-slate-500">Jatuh tempo {dateText(row.dueDate)}</div>}</td><td className="px-5 py-4"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-bold">{retailStatusLabel(row)}</span></td>{documentKind === 'SALES_ORDER' && <td className="px-5 py-4">{row.invoiceSnapshotId ? <button onClick={(event) => { event.stopPropagation(); void openHistory(row); }} className="text-xs font-bold text-emerald-700">{row.commercialStatus==='RETURNED'?'Diretur penuh':row.commercialStatus==='PARTIALLY_RETURNED'?'Diretur sebagian':row.commercialStatus==='RETURN_IN_PROGRESS'?'Retur diproses':'Invoice asli'}</button> : <span className="text-xs text-slate-500">Belum ada Invoice</span>}</td>}<td className="px-5 py-4 text-right"><strong className="block">{money(netCommercialAmount(row.total,credited))}</strong>{credited>0&&<span className="text-xs text-slate-500">Awal {money(row.total)}</span>}</td></tr>})}
    </tbody></table></div></section>
  </div>;
}

function ReturnImpactPanel({ order, returnedLines, creditedAmount, netAmount }: { order: Order; returnedLines: Map<string, SalesReturnCommercialAdjustment['lines'][number]>; creditedAmount: number; netAmount: number }) {
  return <section className="border-t border-violet-200 bg-violet-50/60 p-5">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h3 className="font-black text-violet-950">Dampak Retur</h3><p className="mt-1 text-sm text-violet-800">SO asli tetap menjadi histori. Tabel ini menunjukkan barang aktual kembali dan koreksi tagihannya.</p></div><span className="rounded-full bg-violet-100 px-3 py-1 text-xs font-black text-violet-800">{order.returnAdjustment?.receivedBaseQty ?? 0} base qty diterima</span></div>
    <div className="mt-4 overflow-x-auto rounded-xl border border-violet-200 bg-white"><table className="w-full min-w-[900px] text-sm"><thead className="bg-violet-50 text-left text-xs uppercase text-violet-700"><tr><th className="p-3">Produk</th><th className="p-3 text-right">Qty SO</th><th className="p-3 text-right">Diterima Customer</th><th className="p-3 text-right">Qty kembali</th><th className="p-3 text-right">Sisa di Customer</th><th className="p-3">Disposition</th><th className="p-3 text-right">Kredit</th></tr></thead><tbody>{order.lines.map((line)=>{const returned=returnedLines.get(line.id);const qty=Number(returned?.receivedQtyUom??0);const fulfilled=Number(returned?.fulfilledQtyUom??line.orderedQty);return <tr key={line.id} className="border-t border-violet-100"><td className="p-3"><strong>{line.productCode}</strong><span className="block text-slate-500">{line.productName}</span></td><td className="p-3 text-right">{line.orderedQty} {line.uomCode}</td><td className="p-3 text-right">{fulfilled} {line.uomCode}</td><td className="p-3 text-right font-bold text-violet-700">{qty} {line.uomCode}</td><td className="p-3 text-right font-black">{Math.max(0,fulfilled-qty)} {line.uomCode}</td><td className="p-3">{Number(returned?.restockedBaseQty??0)>0&&<span className="mr-2 inline-flex rounded-full bg-emerald-100 px-2 py-1 text-xs font-bold text-emerald-800">Masuk stok {returned?.restockedBaseQty}</span>}{Number(returned?.destroyedBaseQty??0)>0&&<span className="inline-flex rounded-full bg-rose-100 px-2 py-1 text-xs font-bold text-rose-800">Dihancurkan {returned?.destroyedBaseQty}</span>}{!returned&&<span className="text-slate-400">Tidak diretur</span>}</td><td className="p-3 text-right font-bold">{Number(returned?.creditedAmount??0)>0?`-${money(Number(returned?.creditedAmount))}`:"Menunggu Credit Note"}</td></tr>})}</tbody></table></div>
    <div className="ml-auto mt-4 max-w-md space-y-2 rounded-xl bg-white p-4 text-sm"><div className="flex justify-between"><span>Total SO awal</span><strong>{money(Number(order.grandTotal))}</strong></div><div className="flex justify-between text-violet-700"><span>Credit Note posted</span><strong>-{money(creditedAmount)}</strong></div><div className="flex justify-between border-t pt-3 text-lg font-black"><span>Nilai bersih</span><span>{money(netAmount)}</span></div>{Number(order.returnAdjustment?.receivedBaseQty??0)>0&&creditedAmount===0&&<p className="text-xs text-amber-700">Barang sudah diterima, tetapi tagihan belum berkurang sampai Finance mem-posting Credit Note.</p>}</div>
  </section>
}
...
function OrderDetail({ openSource, session, order, workspace, canCreate, canEdit, canManage, close, edit, createInvoice, createReturn, openReturn, transition, refresh, error }: { openSource: (id: string) => void; session: Session; order: Order; workspace: Workspace; canCreate: boolean; canEdit: boolean; canManage: boolean; close: () => void; edit: () => void; createInvoice: () => void; createReturn: () => void; openReturn: (returnId: string) => void; transition: (order: Order, action: "SEND" | "CONFIRM" | "CANCEL") => Promise<void>; refresh: () => Promise<void>; error: string }) {
  ...
  const returnedLines = new Map((order.returnAdjustment?.lines ?? [])
    .map((line) => [line.sourceLineId, line]));
  const creditedAmount = Number(order.returnAdjustment?.creditedAmount ?? 0);
  const netAmount = netCommercialAmount(Number(order.grandTotal), creditedAmount);
  ...
  return <div className="space-y-3"><DocumentHeader title={order.orderNo ?? order.quotationNo} subtitle={order.orderNo ? `Sales Order · asal ${order.quotationNo}` : "Quotation"} status={order.status} fulfillmentStatus={order.fulfillmentStatus} back={close} actions={actions} />
    ...
    <section className="overflow-hidden border border-slate-200 bg-white shadow-sm">
      ...
      {activeTab === "lines" && order.returnAdjustment && <ReturnImpactPanel order={order} returnedLines={returnedLines} creditedAmount={creditedAmount} netAmount={netAmount}/>}
