...
type Workspace = { companyId: string; defaultWarehouseId: string | null; stores: Store[]; warehouses: Warehouse[]; customers: Customer[]; pricelists?: Pricelist[]; products: ProductUom[] };
...
export function BackofficeSalesOrderView({ session, companyId, companyName, canCreate, canEdit, canManage, notify }: { session: Session; companyId: string; companyName: string; canCreate: boolean; canEdit: boolean; canManage: boolean; notify: (message: string) => void }) {
  ...
  return <div className="space-y-5">
    <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-700">Sales Backoffice</p><h1 className="mt-1 text-3xl font-black">Quotation & Sales Order</h1><p className="mt-1 text-slate-500">Buat Quotation lalu konfirmasi menjadi Sales Order.</p></div>{canCreate && <button disabled={Boolean(missingMaster)} onClick={() => setEditing(null)} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 font-bold text-white disabled:cursor-not-allowed disabled:bg-slate-300"><Plus className="h-5 w-5" />Quotation Baru</button>}</div>
