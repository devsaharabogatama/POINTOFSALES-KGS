...
export function StaffAccessDetailModal({ session, member, close, complete }: { session: Session; member: Staff; close: () => void; complete: () => Promise<void> }) {
  ...
  const [assignmentRoleCode,setAssignmentRoleCode]=useState('COMPANY_ADMIN')
  const [assignmentStoreId,setAssignmentStoreId]=useState('NONE')
  const [selectedRoleCode,setSelectedRoleCode]=useState('COMPANY_ADMIN')
  ...
  async function load(selectedCompanyId?: string) {
    ...
    try {
      ...
      setAssignmentCompanyId(unassigned?.id ?? payload.activeCompanyId)
      setAssignmentStoreId('NONE')
      ...
      setSelectedRoleCode(selectedMembership?.role_code ?? 'COMPANY_ADMIN')
      setSelectedStoreId(selectedStore?.store_id ?? 'NONE')
  ...
  const availableStores=useMemo(()=>detail?.assignmentStores.filter((store)=>store.company_id===assignmentCompanyId) ?? [],[detail,assignmentCompanyId])
  ...
  async function assign() {
    if(!assignmentCompanyId) return
    setSaving(true); setError('')
    try {
      const response=await fetch('/api/staff/assign-existing',{method:'POST',headers:{'Content-Type':'application/json',...authHeaders(session)},body:JSON.stringify({target_user_id:member.id,company_id:assignmentCompanyId,role_code:assignmentRoleCode,store_id:assignmentStoreId})})
      const payload=(await response.json()) as {error?:string}
      if(!response.ok) throw new Error(payload.error ?? 'Gagal menambahkan akses perusahaan')
      await complete(); await load(assignmentCompanyId)
  ...
  async function saveSelectedAccess() {
    ...
    setSaving(true); setError('')
    try {
      const response=await fetch('/api/staff/company-access',{method:'POST',headers:{'Content-Type':'application/json',...authHeaders(session)},body:JSON.stringify({action:'SAVE',targetUserId:member.id,companyId:detail.selectedCompanyId,roleCode:selectedRoleCode,storeId:selectedStoreId})})
      const payload=(await response.json()) as {error?:string}
  ...
  async function deactivateSelectedAccess() {
    ...
    try {
      ...
      setConfirmDeactivate(false); await complete(); await load()
  ...
  return <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/45 p-4 backdrop-blur-sm">
    <div className="max-h-[94vh] w-full max-w-4xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl sm:p-8">
      ...
      {detail&&<div className="mt-7 space-y-7">
        <section><h3 className="flex items-center gap-2 font-black"><Building2 className="h-4 w-4 text-slate-500" />Pilih perusahaan yang diatur</h3><p className="mt-1 text-xs text-slate-500">Role, Toko, dan pembatasan submodul di bawah hanya berlaku untuk perusahaan yang dipilih.</p>{detail.memberships.length===0?<div className="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-5"><p className="font-black text-amber-950">Tidak memiliki akses perusahaan</p><p className="mt-1 text-sm leading-6 text-amber-800">User ini tidak dapat membuka data Company mana pun. Tambahkan akses perusahaan di bawah untuk mengaktifkannya kembali.</p></div>:<div className="mt-3 grid gap-3 md:grid-cols-2">{detail.memberships.map((membership)=><button type="button" onClick={()=>void load(membership.company_id)} key={membership.company_id} className={`rounded-2xl border p-4 text-left transition ${membership.company_id===detail.selectedCompanyId?'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-100':'border-slate-200 hover:border-slate-300'}`}><div className="flex items-start justify-between gap-3"><div><p className="font-bold">{membership.companies?.company_name ?? 'Perusahaan'}</p><p className="mt-1 text-xs text-slate-400">{membership.companies?.company_code}</p></div><div className="flex gap-1">{membership.company_id===detail.selectedCompanyId&&<span className="rounded-full bg-emerald-600 px-2 py-1 text-[10px] font-bold text-white">Sedang diatur</span>}{membership.is_default_company&&<span className="rounded-full bg-blue-50 px-2 py-1 text-[10px] font-bold text-blue-700">Default</span>}</div></div><p className="mt-3 text-sm font-semibold text-emerald-700">{roleLabels[membership.role_code] ?? membership.role_code}</p>{detail.storeMemberships.filter((store)=>store.company_id===membership.company_id).map((store)=><p key={store.store_id} className="mt-2 flex items-center gap-1.5 text-xs text-slate-500"><Store className="h-3.5 w-3.5" />{store.stores?.store_name ?? 'Toko'}</p>)}</button>)}</div>}</section>
        {selectedMembership&&<section className="rounded-2xl border border-emerald-200 bg-emerald-50/50 p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-emerald-700">Mengatur akses untuk</p><h3 className="mt-1 text-lg font-black text-slate-950">{selectedCompany?.company_name}</h3><p className="mt-1 text-xs text-slate-500">Perubahan ini tidak memengaruhi role user pada perusahaan lain.</p></div><button type="button" onClick={()=>setConfirmDeactivate(true)} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-rose-200 bg-white px-3 text-sm font-bold text-rose-700 hover:bg-rose-50"><Trash2 className="h-4 w-4" />Cabut akses</button></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="text-sm font-bold text-slate-700">Role di perusahaan ini<select className="input mt-2" value={selectedRoleCode} onChange={(event)=>{setSelectedRoleCode(event.target.value);if(event.target.value!=='CASHIER')setSelectedStoreId('NONE')}}>{availableRoles.map((role)=><option key={role} value={role}>{roleLabels[role]}</option>)}</select></label><label className="text-sm font-bold text-slate-700">Toko penugasan<select className="input mt-2" value={selectedStoreId} onChange={(event)=>setSelectedStoreId(event.target.value)}><option value="NONE" disabled={selectedRoleCode==='CASHIER'}>{selectedRoleCode==='CASHIER'?'Pilih Toko':'Tanpa Toko spesifik'}</option>{selectedStores.map((store)=><option key={store.id} value={store.id}>{store.store_name}</option>)}</select></label></div><button type="button" disabled={saving||(selectedRoleCode==='CASHIER'&&selectedStoreId==='NONE')} onClick={()=>void saveSelectedAccess()} className="mt-4 inline-flex min-h-11 items-center justify-center rounded-xl bg-emerald-600 px-4 text-sm font-bold text-white disabled:opacity-50">{saving?<Loader2 className="h-4 w-4 animate-spin" />:'Simpan role & Toko'}</button></section>}
        {detail.canAssignOtherCompany&&availableCompanies.length>0&&<section className="rounded-2xl border border-blue-200 bg-blue-50/60 p-5"><h3 className="font-black text-blue-950">Tambahkan ke perusahaan lain</h3><p className="mt-1 text-xs leading-5 text-blue-700">Akun dan password tetap sama. Role hanya berlaku pada perusahaan yang dipilih.</p><div className="mt-4 grid gap-3 md:grid-cols-3"><select className="input" value={assignmentCompanyId} onChange={(event)=>{setAssignmentCompanyId(event.target.value);setAssignmentStoreId('NONE')}}>{availableCompanies.map((company)=><option key={company.id} value={company.id}>{company.company_name}</option>)}</select><select className="input" value={assignmentRoleCode} onChange={(event)=>{setAssignmentRoleCode(event.target.value);setAssignmentStoreId('NONE')}}>{availableRoles.map((role)=><option key={role} value={role}>{roleLabels[role]}</option>)}</select><select className="input" value={assignmentStoreId} onChange={(event)=>setAssignmentStoreId(event.target.value)}><option value="NONE" disabled={assignmentRoleCode==='CASHIER'}>{assignmentRoleCode==='CASHIER'?'Pilih Toko':'Tanpa Toko spesifik'}</option>{availableStores.map((store)=><option key={store.id} value={store.id}>{store.store_name}</option>)}</select></div><button type="button" disabled={saving||!assignmentCompanyId||(assignmentRoleCode==='CASHIER'&&assignmentStoreId==='NONE')} onClick={()=>void assign()} className="mt-4 inline-flex min-h-11 items-center justify-center rounded-xl bg-blue-600 px-4 text-sm font-bold text-white disabled:opacity-50">{saving?<Loader2 className="h-4 w-4 animate-spin" />:'Tambahkan akses'}</button></section>}
        {selectedMembership&&<section><div><h3 className="flex items-center gap-2 font-black"><ShieldCheck className="h-4 w-4 text-slate-500" />Pembatasan submodul — {selectedCompany?.company_name}</h3><p className="mt-1 text-xs text-slate-500">{selectedMembership.role_code==='COMPANY_OWNER'?'Pemilik Perusahaan selalu memiliki seluruh capability yang didukung di perusahaan ini.':'Semua pilihan pada bagian ini hanya berlaku untuk perusahaan terpilih.'}</p></div><div className="mt-3 space-y-2">{permissionGroups.map(([moduleKey,items])=>{const restrictedCount=items.filter((item)=>item.restrictionPreset!=='IKUTI_ROLE').length;return <details key={moduleKey} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white"><summary className="flex cursor-pointer list-none items-center gap-3 px-4 py-4 transition hover:bg-slate-50"><span className="min-w-0 flex-1"><span className="block font-black text-slate-900">{moduleLabels[moduleKey] ?? moduleKey}</span><span className="mt-1 block text-xs text-slate-500">{items.length} submodul{restrictedCount>0?` · ${restrictedCount} dibatasi`:' · seluruhnya mengikuti role'}</span></span>{restrictedCount>0&&<span className="rounded-full bg-amber-50 px-2.5 py-1 text-[10px] font-bold text-amber-700">{restrictedCount} dibatasi</span>}<ChevronDown className="h-4 w-4 shrink-0 text-slate-400 transition-transform group-open:rotate-180" /></summary><div className="divide-y divide-slate-100 border-t border-slate-100 bg-slate-50/40">{items.map((item)=>{const catalog=catalogByKey.get(item.permissionKey);const editable=selectedMembership.role_code!=='COMPANY_OWNER'&&item.enforced&&catalog?.is_customizable;return <div key={item.permissionKey} className="grid gap-2 px-4 py-3 sm:grid-cols-[1fr_auto] sm:items-center"><div><p className="flex items-center gap-2 text-sm font-bold text-slate-800">{catalog?.permission_label ?? item.permissionKey}{item.enforced&&<span className="rounded-full bg-emerald-50 px-2 py-0.5 text-[9px] uppercase tracking-wide text-emerald-700">Aktif</span>}</p><p className="mt-0.5 text-[11px] text-slate-400">{item.effectiveCapabilities.join(', ')||'Tidak ada akses'}</p></div>{editable?<select aria-label={`Pembatasan ${catalog?.permission_label ?? item.permissionKey}`} className="input min-w-44" value={item.restrictionPreset} disabled={savingPermission===item.permissionKey} onChange={(event)=>void updatePermission(item,event.target.value)}>{Object.entries(presetLabels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select>:<span className={`w-fit rounded-full px-2.5 py-1 text-[11px] font-bold ${item.restrictionPreset==='IKUTI_ROLE'?'bg-slate-100 text-slate-600':'bg-amber-50 text-amber-700'}`}>{presetLabels[item.restrictionPreset] ?? item.restrictionPreset}</span>}</div>})}</div></details>})}</div></section>}
