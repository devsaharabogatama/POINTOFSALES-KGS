...
import { Banknote, CheckCircle2, Download, Loader2, Plus, RefreshCcw, Search, X, XCircle } from 'lucide-react'
...
type SourceType = 'RETAIL_SALE' | 'BACKOFFICE_SALES_INVOICE'
type Allocation = { document_id: string; source_type: SourceType; source_id: string; client_allocation_key: string; allocated_amount: number }
type Invoice = { sourceType: SourceType; sourceId: string; salesId: string | null; invoiceNo: string; customerId: string; storeName: string | null; transactionDate: string; dueDate: string | null; originalReceivable: number; allocatedAmount: number; creditedAmount: number; remainingAmount: number }
...
type AgingInvoice = { sourceProcess: 'RETAIL_SALE' | 'BACKOFFICE'; sourceId: string; salesId: string | null; scheduleId: string | null; installmentNo: number | null; installmentCount: number | null; invoiceNo: string; customerId: string; customerCode: string; customerName: string; storeName: string | null; transactionDate: string; dueDate: string | null; originalReceivable: number; allocatedAmount: number; outstanding: number; agingBucket: string; overdueDays: number }
...
type StatementRow = { sequence: number; sourceType: 'INVOICE' | 'RECEIPT' | 'ODR_PAYMENT'; sourceProcess: 'RETAIL' | 'BACKOFFICE'; documentNo: string; businessDate: string; dueDate: string | null; storeName: string | null; description: string; debit: number; credit: number; runningBalance: number }
...
const sourceKey = (row: Pick<Invoice, 'sourceType' | 'sourceId'>) => `${row.sourceType}:${row.sourceId}`
const sourceLabel = (value: SourceType | AgingInvoice['sourceProcess'] | StatementRow['sourceProcess']) => value === 'BACKOFFICE' || value === 'BACKOFFICE_SALES_INVOICE' ? 'Backoffice' : 'Retail'
...
export function CustomerReceiptView({ session, canCreate, canEdit, canPost, canExport }: { session: Session; canCreate: boolean; canEdit: boolean; canPost: boolean; canExport: boolean }) {
  ...
  const selectedAllocations = useMemo(() => invoices.map((invoice) => ({ invoice, amount: Number(amounts[sourceKey(invoice)] ?? 0) })).filter((row) => row.amount > 0), [invoices, amounts])
  ...
  function openCreate() {
    reset(); setError(''); setNotice(''); setFormOpen(true)
  }
  function openDraft(row: Document) {
    ...
    setAmounts(Object.fromEntries(allocations.map((item) => [`${item.source_type}:${item.source_id}`, String(item.allocated_amount)])))
    setKeys(Object.fromEntries(allocations.map((item) => [`${item.source_type}:${item.source_id}`, item.client_allocation_key])))
  ...
  async function saveDraft() {
    ...
    const allocations = disposition === 'NONE' ? selectedAllocations.map(({ invoice, amount }) => ({ sourceType: invoice.sourceType, sourceId: invoice.sourceId, clientAllocationKey: keys[sourceKey(invoice)] ?? crypto.randomUUID(), allocatedAmount: amount })) : []
  ...
  return <div>
    <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-violet-600">Finance · Piutang</p><h1 className="mt-1 text-3xl font-black">Penerimaan Customer</h1><p className="mt-2 text-sm text-slate-600">Catat pembayaran tempo dan alokasikan ke satu atau beberapa invoice Customer.</p></div>{canCreate && <button type="button" onClick={openCreate} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-violet-600 px-4 font-black text-white"><Plus className="h-4 w-4" /> Penerimaan baru</button>}</header>
