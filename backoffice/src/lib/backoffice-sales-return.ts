...
export function parseReturnAllocations(body: JsonObject) {
  ...
  return body.allocations.map((raw) => {
    ...
    if (!['UNINVOICED', 'DRAFT_INVOICE', 'POSTED_INVOICE', 'RETAIL_POSTED_INVOICE'].includes(allocationType)) throw new ApiRouteError("RETURN_INVOICE_ALLOCATION_TYPE_INVALID", 400);
    if (allocationType === 'RETAIL_POSTED_INVOICE') {
      return {
        ...
        retailSalesId: requiredUuid(row, "retailSalesId"),
        retailSalesDetailId: requiredUuid(row, "retailSalesDetailId"),
      };
    }
    return {
      returnReceiptLineId: requiredUuid(row, "returnReceiptLineId"), allocationType,
...
export function throwBackofficeReturnError(error: { code?: string; message?: string } | null): never {
  ...
  const codes = [
    ...
    "RETURN_SOURCE_RETAIL_INVOICE_LINE_INVALID", "RETAINED_RETAIL_SOURCE_INVOICE_REQUIRED", "RETAINED_RETAIL_RETURN_SOURCE_NOT_FOUND",
    "RETAINED_RETAIL_CREDIT_NOTE_NOT_FOUND", "RETAINED_RETAIL_CREDIT_VALUE_INVALID", "CUSTOMER_REFUND_SOURCE_MISMATCH",
    "DRAFT_INVOICE_RETURN_QUANTITY_EXCEEDS_LINE", "CREDIT_NOTE_NOT_FOUND", "CREDIT_NOTE_NOT_EDITABLE", "CREDIT_NOTE_NOT_POSTABLE",
    "CREDIT_NOTE_DATE_FUTURE", "CREDIT_NOTE_DATE_BEFORE_PAYMENT", "CREDIT_NOTE_DELIVERY_FEE_EXCEEDS_SOURCE", "CREDIT_NOTE_AMOUNT_EXCEEDS_INVOICE",
    "CREDIT_NOTE_SOURCE_TAX_ACCOUNT_INVALID", "CUSTOMER_CREDIT_NOTE_CATEGORY_REQUIRED", "POSTABLE_ACCOUNTING_PERIOD_NOT_FOUND",
