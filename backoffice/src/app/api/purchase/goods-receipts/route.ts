...
export async function POST(request:Request){try{const caller=await requireCaller(request);await requireActiveCompany(caller);const input=parseGoodsReceipt(await readJsonObject(request));const {data,error}=await caller.client.rpc('save_generated_backoffice_goods_receipt',{p_document_id:input.documentId,p_master_version:input.masterVersion,p_supplier_order_id:input.supplierOrderId,p_supplier_delivery_no:input.supplierDeliveryNo,p_notes:input.notes,p_lines:input.lines});if(error)throwGoodsReceiptError(error);return Response.json({data})}catch(error){return apiError(error)}}
