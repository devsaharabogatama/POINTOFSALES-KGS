...
export async function GET(request: Request) {
  try {
    ...
    await requirePermissionCapability(
      caller, companyId, 'inventory.stock_real', 'VIEW',
    )
    const { data, error } = await caller.client.rpc('get_inventory_stock_overview')
    if (error) throwDatabaseError(error)
    return Response.json(data ?? { companyId, balances: [], warehouses: [] })
  } catch (error) {
