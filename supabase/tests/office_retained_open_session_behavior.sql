...
BEGIN
 ...
 IF v_result->>'targetStatus'<>'CONFIRMED' THEN RAISE EXCEPTION 'TEST_FAILED: open-session converter invalid: %',v_result; END IF;
 IF EXISTS(SELECT 1 FROM public.sales_order_procurement_demand_lines WHERE sales_id=v_sale_id AND stock_request_line_id IS NOT NULL) THEN RAISE EXCEPTION 'TEST_FAILED: open session prematurely creates request'; END IF;
 ...
 v_payload:=jsonb_build_object('storeId',v_store,'warehouseId',v_warehouse,'customerId',v_customer,
  ...
  'currencyCode','IDR','revisionReason','Open session revision','lines',
 ...
 SELECT master_version INTO STRICT v_version FROM public.cashier_sessions WHERE id=v_session;
 PERFORM public.close_cashier_session(v_session,v_version,0);
 SELECT header.stock_request_document_id INTO STRICT v_request
 FROM public.sales_order_procurement_demands header WHERE company_id=v_company AND cashier_session_id=v_session;
 IF NOT EXISTS(SELECT 1 FROM public.stock_request_lines WHERE document_id=v_request AND requested_base_qty=v_factor AND is_active)
 THEN RAISE EXCEPTION 'TEST_FAILED: session close did not retain revised Office demand'; END IF;
END $test$;
ROLLBACK;
