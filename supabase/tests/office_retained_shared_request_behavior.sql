...
DECLARE
  ...
  v_state_before jsonb;v_state_after jsonb;v_request uuid;v_request_qty numeric;v_original_cap numeric;v_revision_operation uuid;v_version bigint;v_other_sale uuid;
BEGIN
 ...
 IF v_request_qty<>3*v_factor OR NOT EXISTS(SELECT 1 FROM public.sales_order_procurement_demand_lines
 ...
 OR NOT EXISTS(SELECT 1 FROM public.stock_request_lines WHERE company_id=v_company AND document_id=v_request AND is_active AND requested_base_qty=2*v_factor)
