-- Clone rollback fixture derived from audited Invoice posting test.
-- Sequence values may advance despite rollback; no persistent fixture rows.
-- Authenticated rollback-only behavior for Backoffice Invoice posting and DP deduction.
BEGIN;
CREATE FUNCTION pg_temp.assert_so_invoice_status(p_company uuid,p_order uuid,p_expected text,p_draft uuid DEFAULT NULL)
RETURNS void LANGUAGE plpgsql AS $assert$
DECLARE v_summary jsonb;v_rows jsonb;
BEGIN
  v_summary:=private.backoffice_sales_order_invoice_summary(p_company,p_order);
  IF v_summary->>'invoiceStatus' IS DISTINCT FROM p_expected
    OR v_summary->>'draftInvoiceId' IS DISTINCT FROM p_draft::text THEN
    RAISE EXCEPTION 'TEST_FAILED: expected %, summary %',p_expected,v_summary;
  END IF;
  v_rows:=public.get_backoffice_sales_orders_v3('SALES_ORDER',NULL,p_expected,'ORDER_DATE',
    NULL,NULL,(SELECT order_no FROM public.backoffice_sales_orders WHERE company_id=p_company AND id=p_order),100);
  IF NOT EXISTS(SELECT 1 FROM jsonb_array_elements(v_rows->'data') item WHERE item->>'id'=p_order::text
    AND item->>'invoiceStatus'=p_expected AND item->>'draftInvoiceId' IS NOT DISTINCT FROM p_draft::text) THEN
    RAISE EXCEPTION 'TEST_FAILED: filtered SO/link missing: %',v_rows;
  END IF;
END $assert$;
DO $test$
DECLARE
  ...
  v_report jsonb;v_other uuid;v_basis text;v_other_permission jsonb;
...
BEGIN
  ...
  IF NOT EXISTS(SELECT 1 FROM private.kgs_schema_migrations WHERE version='20260912140000') THEN
    RAISE EXCEPTION 'TEST_PRECONDITION_FAILED: unit 72 required';
  END IF;
  ...
  END IF;
  PERFORM pg_temp.assert_so_invoice_status(v_company,v_order,'NOT_READY');
  ...
  END IF;
  PERFORM pg_temp.assert_so_invoice_status(v_company,v_order,'READY');
  v_report:=public.get_finance_delivered_not_invoiced(v_period_date,500,0);
  IF NOT EXISTS(SELECT 1 FROM jsonb_array_elements(v_report->'rows') item
    WHERE item->>'salesOrderId'=v_order::text AND item->>'invoiceStatus'='NOT_DRAFTED'
      AND (item->>'outstandingBaseQty')::numeric=2*v_factor) THEN
    RAISE EXCEPTION 'TEST_FAILED: undrafted real DNI missing: %',v_report;
  END IF;
  -- Receipt may create its own COGS event. Count Invoice-only effects afterward.
  SELECT count(*) INTO v_event_before FROM public.financial_events;
  SELECT count(*) INTO v_journal_before FROM public.finance_journals;

  -- DP must still reject a nonempty overage request after the empty-input fix.
  v_blocked:=false;
  BEGIN
    PERFORM public.save_backoffice_sales_invoice_draft(NULL,NULL,gen_random_uuid(),v_order,
      jsonb_build_object('invoiceType','DOWN_PAYMENT','invoiceDate',v_period_date,
        ...
        'acceptedOverageLines',jsonb_build_array(jsonb_build_object('discrepancyLineId',gen_random_uuid(),'quantityUom',1))));
  EXCEPTION WHEN OTHERS THEN
    IF SQLERRM NOT LIKE '%BACKOFFICE_ACCEPTED_OVERAGE_INVOICE_PAYLOAD_INVALID%' THEN RAISE; END IF;
    v_blocked:=true;
  END;
  IF NOT v_blocked THEN RAISE EXCEPTION 'TEST_FAILED: DP with overage accepted'; END IF;
  v_dp:=public.save_backoffice_sales_invoice_draft(NULL,NULL,gen_random_uuid(),v_order,
    jsonb_build_object('invoiceType','DOWN_PAYMENT','invoiceDate',v_period_date,
      'downPaymentMode','PERCENT','downPaymentInput',40));
  v_dp_id:=(v_dp->'data'->>'id')::uuid;
  PERFORM pg_temp.assert_so_invoice_status(v_company,v_order,'DRAFT',v_dp_id);
  ...
  END IF;
  PERFORM pg_temp.assert_so_invoice_status(v_company,v_order,'PARTIALLY_INVOICED');
  ...
  PERFORM pg_temp.assert_so_invoice_status(v_company,v_order,'DRAFT',v_regular_id);
  v_report:=public.get_finance_delivered_not_invoiced(v_period_date,500,0);
  IF NOT EXISTS(SELECT 1 FROM jsonb_array_elements(v_report->'rows') item
    WHERE item->>'salesOrderId'=v_order::text AND item->>'invoiceStatus'='FULLY_DRAFTED') THEN
    RAISE EXCEPTION 'TEST_FAILED: fully drafted real DNI missing';
  END IF;
  ...
  END IF;
  PERFORM pg_temp.assert_so_invoice_status(v_company,v_order,'INVOICED');
  v_report:=public.get_finance_delivered_not_invoiced(v_period_date,500,0);
  IF EXISTS(SELECT 1 FROM jsonb_array_elements(v_report->'rows') item WHERE item->>'salesOrderId'=v_order::text) THEN
    RAISE EXCEPTION 'TEST_FAILED: Posted SO remains in DNI';
  END IF;
  FOREACH v_basis IN ARRAY ARRAY['ORDER_DATE','DELIVERY_DATE'] LOOP
    v_report:=public.get_backoffice_sales_orders_v3('SALES_ORDER','COMPLETED','INVOICED',
      v_basis,v_period_date,v_period_date,NULL,500);
    IF NOT EXISTS(SELECT 1 FROM jsonb_array_elements(v_report->'data') item WHERE item->>'id'=v_order::text) THEN
      RAISE EXCEPTION 'TEST_FAILED: % filter missing SO',v_basis;
    END IF;
  END LOOP;
  SELECT id INTO v_other FROM public.companies WHERE status='ACTIVE' AND id<>v_company ORDER BY id LIMIT 1;
  IF v_other IS NULL THEN RAISE EXCEPTION 'TEST_PRECONDITION_FAILED: second Company required'; END IF;
  IF private.backoffice_sales_order_invoice_summary(v_other,v_order) IS NOT NULL THEN
    RAISE EXCEPTION 'TEST_FAILED: cross-Company summary leaked';
  END IF;
  UPDATE public.user_active_company_contexts SET company_id=v_other WHERE user_id=v_actor;
  v_other_permission:=private.acp_resolve_permission(v_other,v_actor,'sales.backoffice_orders');
  IF (v_other_permission->>'enforced')::boolean
    AND NOT ((v_other_permission->'effectiveCapabilities') ? 'VIEW') THEN
    v_blocked:=false;
    BEGIN
      PERFORM public.get_backoffice_sales_orders_v3(NULL,NULL,NULL,'ORDER_DATE',NULL,NULL,NULL,500);
    EXCEPTION WHEN OTHERS THEN
      IF SQLERRM NOT LIKE '%CUSTOM_PERMISSION_DENIED%' THEN RAISE; END IF;
      v_blocked:=true;
    END;
    IF NOT v_blocked THEN RAISE EXCEPTION 'TEST_FAILED: resolved tenant VIEW denial bypassed'; END IF;
  ELSE
    v_report:=public.get_backoffice_sales_orders_v3(NULL,NULL,NULL,'ORDER_DATE',NULL,NULL,NULL,500);
    IF EXISTS(SELECT 1 FROM jsonb_array_elements(v_report->'data') item WHERE item->>'id'=v_order::text) THEN
      RAISE EXCEPTION 'TEST_FAILED: cross-Company list leaked';
    END IF;
  END IF;
...
$test$;
ROLLBACK;
SELECT 'backoffice_walkin_payment_clone_behavior' check_name,'PASS' status,
  0::bigint violation_rows,jsonb_build_object('tested',ARRAY[
    'NOT_READY READY DRAFT PARTIALLY_INVOICED INVOICED status/filter/link',
    'real-row DNI undrafted Draft and Posted exit','ORDER/DELIVERY date filters',
    'Company summary/list isolation','canonical taxed SO preparation','stale posting denial','DP Invoice posting',
    ...
    'POS Sales unchanged','WALK-IN allocated-only payment','zero Customer Balance','balanced Customer Receipt journal','payment exact retry','all fixture writes rolled back']) details;
