-- Clone-only owned formatting fixture; all routine rewrites roll back.
BEGIN;
DO $fixture$ DECLARE r record; v_definition text; v_before text; v_raw text; v_lf text; n integer:=0;
BEGIN
...
('private.validate_office_untouched_fulfillment(uuid,uuid)','fc76c9d2bc6724e2376b1ecef1be3b81')
  ...
  v_definition:=pg_get_functiondef(to_regprocedure(r.signature));
  v_before:=md5(v_definition);
  IF v_before<>r.digest THEN RAISE EXCEPTION 'TEST_PRECONDITION_FAILED: %',r.signature; END IF;
  EXECUTE replace(v_definition,chr(10),chr(13)||chr(10));
  v_raw:=md5(pg_get_functiondef(to_regprocedure(r.signature)));
  v_lf:=md5(replace(pg_get_functiondef(to_regprocedure(r.signature)),chr(13)||chr(10),chr(10)));
  IF v_raw=r.digest OR v_lf<>r.digest THEN
   RAISE EXCEPTION 'TEST_FAILED: CRLF reproduction % raw=% normalized=%',r.signature,v_raw,v_lf;
  END IF;
  n:=n+1;
 END LOOP;
 IF n<>15 THEN RAISE EXCEPTION 'TEST_FAILED: expected15 definitions, got%',n; END IF;
END $fixture$;
ROLLBACK;
