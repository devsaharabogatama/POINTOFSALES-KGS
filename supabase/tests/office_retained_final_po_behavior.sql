...
DECLARE
  ...
  v_state_before jsonb;v_state_after jsonb;v_request uuid;v_request_qty numeric;v_original_cap numeric;v_revision_operation uuid;v_version bigint;v_other_sale uuid;v_supplier uuid:=gen_random_uuid();v_po uuid:=gen_random_uuid();v_po_line uuid:=gen_random_uuid();v_po_before jsonb;v_base_uom uuid;
BEGIN
 ...
 SELECT uom_id INTO STRICT v_base_uom FROM public.product_uoms WHERE id=v_product_uom;
 UPDATE public.product_uoms SET purchase_allowed=true WHERE id=v_product_uom;
 UPDATE public.warehouses SET is_purchase_destination=true WHERE id=v_warehouse;
 INSERT INTO public.suppliers(id,company_id,supplier_code,supplier_name,created_by,updated_by)
 VALUES(v_supplier,v_company,'TST-COMMIT-'||substr(v_supplier::text,1,8),'Rollback-only committed supplier',v_actor,v_actor);
 INSERT INTO public.supplier_order_documents(id,company_id,order_no,store_id,destination_warehouse_id,supplier_id,order_date,
 ordered_by,line_count,total_ordered_base_qty,estimated_total)
 VALUES(v_po,v_company,'TST-COMMIT-'||v_po::text,v_store,v_warehouse,v_supplier,v_today,v_actor,1,4*v_factor,0);
 INSERT INTO public.supplier_order_lines(id,company_id,document_id,line_no,client_line_key,product_id,ordered_uom_id,
 ordered_qty,factor_to_base_snapshot,ordered_base_qty,estimated_unit_price,estimated_subtotal,
 product_sku_snapshot,product_name_snapshot,ordered_uom_name_snapshot,source_warehouse_id,destination_warehouse_id)
 VALUES(v_po_line,v_company,v_po,1,gen_random_uuid(),v_product,v_base_uom,4,v_factor,4*v_factor,0,0,
 'COMMIT-FIXTURE','Committed fixture','Canonical UOM',v_warehouse,v_warehouse);
 INSERT INTO public.supplier_order_request_allocations(company_id,supplier_order_line_id,stock_request_line_id,allocated_base_qty,created_by)
 SELECT v_company,v_po_line,id,4*v_factor,v_actor FROM public.stock_request_lines WHERE document_id=v_request AND product_id=v_product;
 PERFORM public.confirm_supplier_order(v_po,1,gen_random_uuid());
 SELECT jsonb_build_object('po',to_jsonb(header),'lines',(SELECT jsonb_agg(to_jsonb(line) ORDER BY id) FROM public.supplier_order_lines line WHERE document_id=v_po),
 'allocations',(SELECT jsonb_agg(to_jsonb(allocation) ORDER BY id) FROM public.supplier_order_request_allocations allocation WHERE supplier_order_line_id=v_po_line))
 INTO v_po_before FROM public.supplier_order_documents header WHERE id=v_po;
 ...
 IF v_request_qty<>4*v_factor OR NOT EXISTS(SELECT 1 FROM public.sales_order_procurement_demand_lines
 ...
 PERFORM public.save_backoffice_sales_order_draft(v_target,v_version,gen_random_uuid(),v_payload);
 SELECT requested_base_qty INTO STRICT v_request_qty FROM public.stock_request_lines WHERE company_id=v_company AND document_id=v_request AND product_id=v_product;
 IF v_request_qty<>v_original_cap+2*v_factor THEN RAISE EXCEPTION 'TEST_FAILED: reinstated original obligation invalid'; END IF;
 v_payload:=jsonb_set(v_payload,'{lines,0,quantity}','3'::jsonb);
 SELECT master_version INTO STRICT v_version FROM public.backoffice_sales_orders WHERE company_id=v_company AND id=v_target;
 PERFORM public.save_backoffice_sales_order_draft(v_target,v_version,gen_random_uuid(),v_payload);
 IF (SELECT requested_base_qty FROM public.stock_request_lines WHERE company_id=v_company AND document_id=v_request AND product_id=v_product)<>v_original_cap+2*v_factor
 ...
 OR NOT EXISTS(SELECT 1 FROM public.stock_request_lines WHERE company_id=v_company AND document_id=v_request AND is_active AND requested_base_qty=4*v_factor)
 OR NOT EXISTS(SELECT 1 FROM public.sales_order_procurement_demand_lines WHERE company_id=v_company AND sales_id=v_other_sale AND released_base_qty=0 AND demand_base_qty=2*v_factor)
 OR NOT EXISTS(SELECT 1 FROM public.sales_headers WHERE company_id=v_company AND id=v_other_sale AND order_runtime_status='RESERVED')
 ...
 SELECT jsonb_build_object('po',to_jsonb(header),'lines',(SELECT jsonb_agg(to_jsonb(line) ORDER BY id) FROM public.supplier_order_lines line WHERE document_id=v_po),
 'allocations',(SELECT jsonb_agg(to_jsonb(allocation) ORDER BY id) FROM public.supplier_order_request_allocations allocation WHERE supplier_order_line_id=v_po_line))
 INTO v_state_after FROM public.supplier_order_documents header WHERE id=v_po;
 IF v_po_before IS DISTINCT FROM v_state_after THEN RAISE EXCEPTION 'TEST_FAILED: revised/canceled SO mutated committed PO'; END IF;
 IF NOT EXISTS(SELECT 1 FROM public.sales_order_procurement_amendments WHERE company_id=v_company
 AND stock_request_document_id=v_request AND reason='FINAL_PO_IMMUTABLE' AND delta_base_qty=-2*v_factor)
 THEN RAISE EXCEPTION 'TEST_FAILED: final commitment variance not traced'; END IF;
