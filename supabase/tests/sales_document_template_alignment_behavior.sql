-- Rollback-safe behavior for Delivery signature template setting.
BEGIN;
DO $test$
...
BEGIN
  ...
  v_branding:=public.save_company_document_visibility(
    ...
    COALESCE(v_branding->>'invoiceDateDisplayMode','ORDER_DATE'),'STORE');
  IF v_branding->>'deliverySignatureTemplate'<>'STORE' THEN
    RAISE EXCEPTION 'TEST_FAILED: Store Delivery template not saved';
  END IF;
  ...
  BEGIN
    PERFORM public.save_company_document_visibility(
      v_version,TRUE,FALSE,FALSE,'ORDER_DATE','INVALID');
    RAISE EXCEPTION 'TEST_FAILED: invalid Delivery template accepted';
  EXCEPTION WHEN OTHERS THEN
    IF SQLERRM NOT LIKE '%DELIVERY_SIGNATURE_TEMPLATE_INVALID%' THEN RAISE; END IF;
  END;
END
$test$;
ROLLBACK;
