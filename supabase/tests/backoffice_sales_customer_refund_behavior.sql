-- Authenticated rollback-only behavior for Backoffice Sales Return Step 4/5.
DO $test$
DECLARE
  ...
  v_statement jsonb;v_refund_result jsonb;v_refund_retry jsonb;
  v_refund_id uuid;v_refund_amount numeric(24,4);v_liability numeric(24,4);
  v_refund_op uuid:=gen_random_uuid();v_reversal_op uuid:=gen_random_uuid();
  v_reversal_result jsonb;v_reversal_retry jsonb;
  v_before_session bigint;v_after_session bigint;
...
BEGIN
  ...
  IF (SELECT count(*) FROM private.kgs_schema_migrations
    WHERE version IN('20260917150000','20260917151000'))<>2 THEN
    RAISE EXCEPTION 'TEST_PRECONDITION_FAILED: Step 4 Refund migration and reversal guard fix required';
  END IF;
  ...
  v_phase:='PARTIAL_REFUND_POST';
  SELECT refund_liability_amount,master_version INTO STRICT v_liability,v_version
  FROM public.backoffice_sales_credit_notes
  WHERE company_id=v_company AND id=v_note;
  v_refund_amount:=round(v_liability/2,4);
  IF v_refund_amount<=0 OR v_refund_amount>=v_liability THEN
    RAISE EXCEPTION 'TEST_FAILED: fixture Refund liability cannot exercise partial settlement';
  END IF;
  v_failed:=false;
  BEGIN
    PERFORM public.post_backoffice_sales_customer_refund(v_note,v_version+1,
      gen_random_uuid(),v_today,v_refund_amount,v_method,'RETURN-REFUND-STALE',
      CASE WHEN v_proof_mode='REQUIRED'
        THEN 'https://example.invalid/rollback-only-refund-proof' ELSE NULL END,
      'Step 4 stale version rejection');
  EXCEPTION WHEN OTHERS THEN
    v_failed:=SQLERRM LIKE '%MASTER_VERSION_CONFLICT%';
  END;
  IF NOT v_failed THEN
    RAISE EXCEPTION 'TEST_FAILED: stale Credit Note version accepted for Refund';
  END IF;
  SELECT count(*) INTO v_before_session FROM public.cashier_sessions
  WHERE company_id=v_company;
  v_refund_result:=public.post_backoffice_sales_customer_refund(v_note,v_version,
    v_refund_op,v_today,v_refund_amount,v_method,'RETURN-REFUND-PARTIAL',
    CASE WHEN v_proof_mode='REQUIRED'
      THEN 'https://example.invalid/rollback-only-refund-proof' ELSE NULL END,
    'Step 4 partial Refund fixture');
  v_refund_id:=(v_refund_result->'data'->>'id')::uuid;
  v_refund_retry:=public.post_backoffice_sales_customer_refund(v_note,v_version,
    v_refund_op,v_today,v_refund_amount,v_method,'RETURN-REFUND-PARTIAL',
    CASE WHEN v_proof_mode='REQUIRED'
      THEN 'https://example.invalid/rollback-only-refund-proof' ELSE NULL END,
    'Step 4 partial Refund fixture');
  IF COALESCE((v_refund_retry->>'exactRetry')::boolean,false) IS NOT TRUE THEN
    RAISE EXCEPTION 'TEST_FAILED: partial Refund exact retry rejected';
  END IF;
  v_failed:=false;
  BEGIN
    UPDATE public.backoffice_sales_customer_refunds SET notes='MUTATION MUST FAIL'
    WHERE company_id=v_company AND id=v_refund_id;
  EXCEPTION WHEN OTHERS THEN
    v_failed:=SQLERRM LIKE '%BACKOFFICE_SALES_CUSTOMER_REFUND_HISTORY_IMMUTABLE%';
  END;
  IF NOT v_failed THEN
    RAISE EXCEPTION 'TEST_FAILED: posted Refund history mutable';
  END IF;

  v_phase:='REFUND_OVER_ALLOCATION';
  v_failed:=false;
  BEGIN
    PERFORM public.post_backoffice_sales_customer_refund(v_note,v_version,
      gen_random_uuid(),v_today,v_liability,v_method,'RETURN-REFUND-OVER',
      CASE WHEN v_proof_mode='REQUIRED'
        THEN 'https://example.invalid/rollback-only-refund-proof' ELSE NULL END,
      'Step 4 over-allocation rejection');
  EXCEPTION WHEN OTHERS THEN
    v_failed:=SQLERRM LIKE '%CUSTOMER_REFUND_AMOUNT_EXCEEDS_LIABILITY%';
  END;
  IF NOT v_failed THEN
    RAISE EXCEPTION 'TEST_FAILED: Refund over-allocation accepted';
  END IF;

  v_phase:='SECOND_PARTIAL_REFUND';
  PERFORM public.post_backoffice_sales_customer_refund(v_note,v_version,
    gen_random_uuid(),v_today,round(v_liability-v_refund_amount,4),v_method,
    'RETURN-REFUND-REMAINDER',
    CASE WHEN v_proof_mode='REQUIRED'
      THEN 'https://example.invalid/rollback-only-refund-proof' ELSE NULL END,
    'Step 4 remaining Refund fixture');
  IF NOT EXISTS(SELECT 1 FROM public.backoffice_sales_returns document
    WHERE document.company_id=v_company AND document.id=v_return
      AND document.status='COMPLETED') THEN
    RAISE EXCEPTION 'TEST_FAILED: fully settled Return not completed';
  END IF;

  v_phase:='SOURCE_LINKED_REVERSAL';
  v_reversal_result:=public.reverse_backoffice_sales_customer_refund(v_refund_id,1,
    v_reversal_op,v_today,'Rollback-only correction of first partial Refund');
  v_reversal_retry:=public.reverse_backoffice_sales_customer_refund(v_refund_id,1,
    v_reversal_op,v_today,'Rollback-only correction of first partial Refund');
  IF COALESCE((v_reversal_retry->>'exactRetry')::boolean,false) IS NOT TRUE
    OR NOT EXISTS(SELECT 1 FROM public.backoffice_sales_returns document
      WHERE document.company_id=v_company AND document.id=v_return
        AND document.status='REFUND_PENDING') THEN
    RAISE EXCEPTION 'TEST_FAILED: Refund reversal or status reopening invalid';
  END IF;
  SELECT count(*) INTO v_after_session FROM public.cashier_sessions
  WHERE company_id=v_company;
  IF v_after_session<>v_before_session THEN
    RAISE EXCEPTION 'TEST_FAILED: Backoffice Refund mutated Cashier Session';
  END IF;
  ...
  IF COALESCE((v_retry->>'exactRetry')::boolean,false) IS NOT TRUE
    ...
    OR NOT EXISTS(SELECT 1 FROM jsonb_array_elements(v_statement->'rows') item
      WHERE item->>'sourceType'='CUSTOMER_REFUND'
        AND item->>'sourceProcess'='BACKOFFICE')
    OR NOT EXISTS(SELECT 1 FROM public.finance_journals journal
      ...
      WHERE note.company_id=v_company AND note.id=v_note AND journal.status='POSTED'
        AND journal.total_debit=note.grand_total AND journal.total_credit=note.grand_total)
    OR (SELECT count(*) FROM public.backoffice_sales_customer_refunds refund
        WHERE refund.company_id=v_company AND refund.credit_note_id=v_note)<>3
    OR (SELECT count(*) FROM public.finance_journals journal
      JOIN public.backoffice_sales_customer_refunds refund
        ON refund.company_id=journal.company_id
       AND refund.financial_event_id=journal.financial_event_id
      WHERE refund.company_id=v_company AND refund.credit_note_id=v_note
        AND journal.status='POSTED' AND journal.total_debit=refund.amount
        AND journal.total_credit=refund.amount)<>3 THEN
    RAISE EXCEPTION 'TEST_FAILED: Credit Note, Refund, reversal, statement or Journal invalid';
  END IF;
  ...
  EXCEPTION WHEN SQLSTATE 'P7701' THEN
    NULL;
  END;
  RAISE NOTICE 'backoffice_sales_customer_refund_behavior PASS: partial settlement, cap, exact retry, reversal, statement and no Cashier Session mutation; all fixture writes rolled back';
EXCEPTION WHEN OTHERS THEN
