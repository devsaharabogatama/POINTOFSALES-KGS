-- Authenticated rollback-only behavior for Step 4/6.5C4 discrepancy client read model.
BEGIN;
DO $test$
DECLARE
  ...
  v_approved jsonb;v_retry jsonb;v_payload jsonb;v_disposition jsonb;v_workspace jsonb;
...
BEGIN
  IF NOT EXISTS(SELECT 1 FROM private.kgs_schema_migrations
    ...
    RAISE EXCEPTION 'TEST_PRECONDITION_FAILED: discrepancy client read model migration required';
  END IF;
  ...
  v_workspace:=public.get_backoffice_sales_discrepancy_workspace(v_order_id,NULL);
  IF v_workspace->>'workspaceVersion'<>'1'
    OR jsonb_array_length(v_workspace->'cases')<>1
    OR jsonb_array_length(v_workspace->'lines')<>3
    OR (v_workspace->'cases'->0->>'discrepancyNo') IS NULL
    OR v_workspace->'cases'->0->>'status'<>'PENDING_SALES_APPROVAL' THEN
    RAISE EXCEPTION 'TEST_FAILED: Sales SO discrepancy workspace invalid %',v_workspace;
  END IF;
  v_workspace:=public.get_backoffice_sales_discrepancy_workspace(NULL,v_delivery_id);
  IF jsonb_array_length(v_workspace->'cases')<>1
    OR jsonb_array_length(v_workspace->'lines')<>3
    OR NOT EXISTS(SELECT 1 FROM jsonb_array_elements(v_workspace->'productUoms') item
      WHERE item->>'productId'=v_product::text)
    OR jsonb_array_length(v_workspace->'taxRules')<>0
    OR EXISTS(SELECT 1 FROM jsonb_array_elements(v_workspace->'lines') item
      WHERE item->'sourceUnitPrice'<>'null'::jsonb
        OR item->'approvedUnitPrice'<>'null'::jsonb
        OR item->'commercialSnapshot'<>'null'::jsonb) THEN
    RAISE EXCEPTION 'TEST_FAILED: Warehouse DO discrepancy workspace invalid %',v_workspace;
  END IF;
  ...
  END IF;
  v_workspace:=public.get_backoffice_sales_discrepancy_workspace(v_order_id,NULL);
  IF v_workspace->'cases'->0->>'status'<>'PENDING_WAREHOUSE_RESOLUTION'
    OR (SELECT count(*) FROM jsonb_array_elements(v_workspace->'lines') item
      WHERE item->>'commercialApprovalStatus'='APPROVED')<>2 THEN
    RAISE EXCEPTION 'TEST_FAILED: post-approval client projection invalid %',v_workspace;
  END IF;
...
$test$;
ROLLBACK;
SELECT 'backoffice_sales_discrepancy_client_read_model_behavior' check_name,
  'PASS' status,0::bigint violation_rows,jsonb_build_object('tested',ARRAY[
    ...
    'SO and DO client projection','post-approval projection','Product-UOM options',
