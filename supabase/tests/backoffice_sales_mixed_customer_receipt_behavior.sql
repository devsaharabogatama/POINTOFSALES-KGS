-- Authenticated rollback-only behavior for mixed Backoffice Customer receipt.
BEGIN;
DO $test$
DECLARE
...
BEGIN
  IF NOT EXISTS(SELECT 1 FROM private.kgs_schema_migrations
    ...
    RAISE EXCEPTION 'TEST_PRECONDITION_FAILED: mixed Customer receipt runtime required';
  END IF;
  ...
  -- Rollback-only Company-mode setup; operational RPCs retain their real gates.
...
$test$;
ROLLBACK;
SELECT 'backoffice_sales_mixed_customer_receipt_behavior' check_name,
  'PASS' status,0::bigint violation_rows,jsonb_build_object('tested',ARRAY[
    ...
    'changed retry payload rejected','all fixture writes rolled back']) details;
