-- G6 phase 8G final controlled live operation: post exactly 2/2/2/1 events.
...
DECLARE
 ...
 v_stock BIGINT; v_expense BIGINT; v_deposit BIGINT; v_variance BIGINT;
 v_expected_hash TEXT; v_preview JSONB; v_approval JSONB; v_process JSONB;
BEGIN
 SELECT count(DISTINCT event.company_id),min(event.company_id::TEXT)::UUID,
  count(*) FILTER(WHERE event.system_event_key='STOCK_GAIN'),
  count(*) FILTER(WHERE event.system_event_key='EXPENSE_DISBURSEMENT'),
  count(*) FILTER(WHERE event.system_event_key='CASH_DEPOSIT'),
  count(*) FILTER(WHERE event.system_event_key='CASH_VARIANCE')
 INTO v_company_count,v_company,v_stock,v_expense,v_deposit,v_variance
 ...
 IF v_company_count<>1 OR v_stock<>2 OR v_expense<>2
  OR v_deposit<>2 OR v_variance<>1 THEN
  RAISE EXCEPTION
   'LIVE_SCOPE_CHANGED: expected one Company and 2/2/2/1 events; got companies %, Stock %, Expense %, Deposit %, Variance %',
   v_company_count,v_stock,v_expense,v_deposit,v_variance;
 END IF;
 ...
 IF (v_preview->>'eventCount')::BIGINT<>7
 ...
 IF v_process->>'status'<>'COMPLETED'
  OR (v_process->>'postedCount')::BIGINT<>7
 ...
 RAISE NOTICE 'CONTROLLED LIVE REMAINING OPERATIONAL RESULT: %',v_process;
END
$live$;
COMMIT;
...
WHERE run.scope_system_key='REMAINING_OPERATIONAL'
