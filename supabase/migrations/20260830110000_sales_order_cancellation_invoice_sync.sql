-- Synchronize Sales Order cancellation across POS, Backoffice, Invoice/SJ,
-- Reservation, procurement demand, and unresolved payment intents.
...
BEGIN
  IF (SELECT count(*) FROM private.kgs_schema_migrations
      WHERE version IN('20260828240000','20260828270000','20260828280000'))<>3 THEN
    RAISE EXCEPTION 'MIGRATION_PRECONDITION_FAILED: ODR payment/cancel/invoice runtime required';
  END IF;
  IF EXISTS(SELECT 1 FROM private.kgs_schema_migrations
      WHERE version='20260830110000') THEN
    RAISE EXCEPTION 'MIGRATION_ALREADY_APPLIED: 20260830110000';
  END IF;
  ...
  INTO v_definition;
  IF v_definition IS NULL OR v_definition!~'refresh_sales_order_procurement_demand'
    OR v_definition!~'cancel_confirmed_order_delivery' THEN
    RAISE EXCEPTION 'MIGRATION_PRECONDITION_FAILED: canonical cancellation composition changed';
  END IF;
END
$guard$;

CREATE FUNCTION private.cancel_pending_sales_order_payments(
...
DECLARE v_request public.sales_payment_verification_requests%ROWTYPE;
  v_session_status public.session_status;v_reversal UUID;v_expected NUMERIC(24,4);
  v_before JSONB;v_after JSONB;v_count BIGINT:=0;v_cash_count BIGINT:=0;
...
BEGIN
  ...
  LOOP
    ...
    IF v_request.settlement_route_snapshot='CASH_DRAWER' THEN
      ...
      IF v_session_status IS DISTINCT FROM 'OPEN'::public.session_status THEN
        RAISE EXCEPTION 'SALES_ORDER_CASH_REFUND_REQUIRES_OPEN_SESSION';
      END IF;
      ...
      IF v_reversal IS NULL THEN
        v_expected:=private.calculate_cashier_session_expected_cash(
          p_company_id,v_request.cashier_session_id)-v_request.amount;
        ...
        VALUES(p_company_id,v_request.store_id,v_request.pos_terminal_id,
...
DECLARE v_company UUID:=public.private_active_company_id();v_actor UUID:=auth.uid();
  v_payment JSONB;v_result JSONB;
BEGIN
  ...
  v_payment:=private.cancel_pending_sales_order_payments(v_company,p_sales_id,
    v_actor,p_idempotency_key,p_reason);
  v_result:=private.odr6a_cancel_pos_sales_order_legacy(
    p_sales_id,p_master_version,p_idempotency_key,p_reason);
  RETURN v_result||jsonb_build_object('paymentResolution',v_payment);
END
$$;

CREATE FUNCTION public.cancel_sales_order_from_backoffice(
  p_sales_id UUID,p_master_version BIGINT,p_idempotency_key UUID,p_reason TEXT
) RETURNS JSONB LANGUAGE plpgsql SECURITY DEFINER
SET search_path=public,pg_temp AS $$
DECLARE v_company UUID:=public.private_active_company_id();v_actor UUID:=auth.uid();
...
BEGIN
  ...
  IF NOT ((v_permission->'effectiveCapabilities') ? 'CANCEL_FINAL') THEN
    RAISE EXCEPTION 'CUSTOM_PERMISSION_DENIED';
  END IF;
  RETURN public.cancel_pos_sales_order(p_sales_id,p_master_version,
...
BEGIN
  ...
  RETURN COALESCE((SELECT jsonb_agg(jsonb_build_object(
    ...
    'customerName',COALESCE(customer.name,'Walk-In Customer'),
    'storeName',COALESCE(store.store_name,'Store'),
    'sourceChannel',sale.source_channel,'fulfillmentMode',sale.fulfillment_mode,
    'grandTotal',sale.grand_total_after_rounding,'deliveryFee',sale.delivery_fee_amount,
    'canceledAt',sale.canceled_at,'cancelReason',sale.cancel_reason,
    'canceledByName',cancel_actor.name,'snapshotProvenance',invoice.snapshot_provenance)
    ORDER BY invoice.created_at DESC,invoice.id)
...
TO service_role;
REVOKE ALL ON FUNCTION
  public.cancel_sales_order_from_backoffice(UUID,BIGINT,UUID,TEXT)
FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION
  public.cancel_sales_order_from_backoffice(UUID,BIGINT,UUID,TEXT)
...
VALUES('20260830110000','sales_order_cancellation_invoice_sync',
  'Canonical POS/Backoffice cancellation, pending payment intent cancellation and open-drawer reversal, immutable canceled Invoice projection/export, Reservation/SJ/procurement composition preserved; verified payments and closed-drawer refunds remain fail-closed');
