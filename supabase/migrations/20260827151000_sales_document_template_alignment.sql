-- Align POS A4 Invoice with Backoffice and add immutable Delivery signature template.
BEGIN;
...
BEGIN
  IF NOT EXISTS(SELECT 1 FROM private.kgs_schema_migrations
    ...
    RAISE EXCEPTION 'MIGRATION_PRECONDITION_FAILED: 20260827150000 required';
  END IF;
  IF EXISTS(SELECT 1 FROM private.kgs_schema_migrations
    WHERE version='20260827151000') THEN
    RAISE EXCEPTION 'MIGRATION_ALREADY_APPLIED: 20260827151000';
  END IF;
END
$guard$;

ALTER TABLE public.company_branding_profiles
  ADD COLUMN delivery_signature_template TEXT NOT NULL DEFAULT 'WAREHOUSE',
  ADD CONSTRAINT company_branding_delivery_signature_template_check
    CHECK(delivery_signature_template IN('WAREHOUSE','STORE'));
...
BEGIN
  ...
  IF NOT FOUND THEN
    ...
    INSERT INTO public.company_branding_profiles(company_id,show_logo_on_documents,
      ...
      delivery_signature_template,updated_by,updated_at)
    VALUES(v_company,p_show_logo_on_documents,p_show_stamp_on_documents,
      p_show_bank_account_on_invoice,v_date_mode,v_delivery_template,v_actor,
      clock_timestamp());
  ELSE
    IF v_existing.show_logo_on_documents=p_show_logo_on_documents
      ...
      AND v_existing.delivery_signature_template=v_delivery_template THEN
...
SET search_path=public,pg_temp AS $$
DECLARE v_company UUID:=public.private_active_company_id();v_template TEXT;
BEGIN
  SELECT profile.delivery_signature_template INTO v_template
...
BEGIN
  ...
  v_snapshot:=jsonb_set(v_snapshot,'{branding}',COALESCE(v_snapshot->'branding','{}'::JSONB)
    ||jsonb_build_object(
      'showLogoOnDocuments',COALESCE(v_branding.show_logo_on_documents,TRUE),
      'showStampOnDocuments',COALESCE(v_branding.show_stamp_on_documents,FALSE),
      'showBankAccountOnInvoice',COALESCE(v_branding.show_bank_account_on_invoice,FALSE),
      'invoiceDateDisplayMode',COALESCE(v_branding.invoice_date_display_mode,'ORDER_DATE'),
      'deliverySignatureTemplate',COALESCE(v_branding.delivery_signature_template,'WAREHOUSE')),TRUE);
  RETURN v_snapshot;
END
$$;

REVOKE ALL ON FUNCTION public.save_company_document_visibility(
  BIGINT,BOOLEAN,BOOLEAN,BOOLEAN,TEXT,TEXT) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.save_company_document_visibility(
  BIGINT,BOOLEAN,BOOLEAN,BOOLEAN,TEXT,TEXT) TO authenticated,service_role;
...
VALUES('20260827151000','sales_document_template_alignment',
  'Snapshots Invoice presentation switches and a per-Company Warehouse or Store Delivery signature template so POS and Backoffice render the same new final documents');
