-- Step 4/6.5B forward fix: align resolver audit INSERT to the canonical
-- backoffice_sales_discrepancy_audit schema. The note remains in immutable
-- operation.request_payload and is not duplicated into a nonexistent column.
BEGIN;
...
BEGIN
  IF NOT EXISTS(SELECT 1 FROM private.kgs_schema_migrations
    ...
    RAISE EXCEPTION 'MIGRATION_PRECONDITION_FAILED: shortage resolution runtime required';
  END IF;
  IF EXISTS(SELECT 1 FROM private.kgs_schema_migrations
    WHERE version='20260912121000') THEN
    RAISE EXCEPTION 'MIGRATION_ALREADY_APPLIED: 20260912121000';
  END IF;
  IF (SELECT count(*) FROM information_schema.columns
      WHERE table_schema='public' AND table_name='backoffice_sales_discrepancy_audit'
        AND column_name IN('company_id','discrepancy_id','operation_id','action',
          'actor_id','before_state','after_state'))<>7
    OR EXISTS(SELECT 1 FROM information_schema.columns
      WHERE table_schema='public' AND table_name='backoffice_sales_discrepancy_audit'
        AND column_name='reason') THEN
    RAISE EXCEPTION 'MIGRATION_PRECONDITION_FAILED: discrepancy audit schema drift';
  END IF;
END
$guard$;

CREATE OR REPLACE FUNCTION private.resolve_backoffice_sales_shortage_core(
...
VALUES('20260912121000','backoffice_sales_shortage_resolution_audit_fix',
  'Forward-fixes the Warehouse shortage resolver to persist notes in immutable operation request_payload and use only canonical discrepancy audit columns');
