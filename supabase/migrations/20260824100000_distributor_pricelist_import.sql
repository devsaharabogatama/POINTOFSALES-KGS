-- Controlled distributor Pricelist import by Company + SKU.
-- Source prices are per PACK. Every active Product-UOM price is derived from
-- its factor_to_base, so DUS and other sales/purchase UOMs remain consistent.
...
BEGIN
  IF NOT EXISTS(
    ...
    RAISE EXCEPTION 'MIGRATION_PRECONDITION_FAILED: latest platform foundation required';
  END IF;
  IF EXISTS(
    SELECT 1 FROM private.kgs_schema_migrations
  ...
  IF (SELECT count(*) FROM public.access_permission_catalog
      WHERE permission_key='sales.pricelists' AND enforcement_status='ENFORCED')<>1 THEN
    RAISE EXCEPTION 'PRICELIST_PERMISSION_NOT_ENFORCED';
  END IF;
END
$guard$;

CREATE TABLE public.pricelist_import_runs(
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  company_id UUID NOT NULL REFERENCES public.companies(id) ON DELETE RESTRICT,
  client_request_id UUID NOT NULL,
  source_file_name TEXT NOT NULL,
  source_file_checksum TEXT NOT NULL,
  import_kind TEXT NOT NULL DEFAULT 'DISTRIBUTOR_PACK_PRICE',
  status TEXT NOT NULL DEFAULT 'COMMITTED',
  source_row_count INTEGER NOT NULL,
  applied_product_count INTEGER NOT NULL,
  applied_pricelist_count INTEGER NOT NULL,
  result_snapshot JSONB NOT NULL,
  actor_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE RESTRICT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp(),
  CONSTRAINT pricelist_import_run_identity UNIQUE(company_id,client_request_id),
  CONSTRAINT pricelist_import_checksum_shape CHECK(source_file_checksum~'^[0-9a-f]{64}$'),
  CONSTRAINT pricelist_import_kind_check CHECK(import_kind='DISTRIBUTOR_PACK_PRICE'),
  CONSTRAINT pricelist_import_status_check CHECK(status='COMMITTED'),
  CONSTRAINT pricelist_import_count_check CHECK(
    source_row_count>0 AND applied_product_count>=0 AND applied_pricelist_count>=0
  )
);

CREATE INDEX idx_pricelist_import_runs_company_created
ON public.pricelist_import_runs(company_id,created_at DESC);

ALTER TABLE public.pricelist_import_runs ENABLE ROW LEVEL SECURITY;
REVOKE ALL ON public.pricelist_import_runs FROM PUBLIC,anon,authenticated;
GRANT ALL ON public.pricelist_import_runs TO service_role;

CREATE FUNCTION private.trg_guard_pricelist_import_history()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER
SET search_path=public,pg_temp AS $$
BEGIN
  RAISE EXCEPTION 'PRICELIST_IMPORT_HISTORY_IMMUTABLE';
END
$$;

CREATE TRIGGER trg_guard_pricelist_import_history
BEFORE UPDATE OR DELETE ON public.pricelist_import_runs
FOR EACH ROW EXECUTE FUNCTION private.trg_guard_pricelist_import_history();

CREATE FUNCTION public.process_distributor_pricelist_import(
  p_rows JSONB,
...
UPDATE public.access_permission_catalog SET
  supported_capabilities=array_append(supported_capabilities,'IMPORT'),
  catalog_version=catalog_version+1,updated_at=clock_timestamp()
WHERE permission_key='sales.pricelists'
  AND NOT('IMPORT'=ANY(supported_capabilities));
...
VALUES('20260824100000','distributor_pricelist_import',
  'Controlled Company+SKU distributor price import with PACK-derived Product-UOM prices, customer Pricelists, Global quantity tiers, preview and exact replay');
