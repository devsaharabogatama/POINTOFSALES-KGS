-- G4 phase 5 forward fix: a Store Manager assigned to a Store may operate
-- that Store's POS, as approved by the POS role matrix.
--
-- Dependency: the Super/Company Admin inheritance fix.
-- No table, business data, stock, Sale, or Session history is changed.
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION
            'MIGRATION_PRECONDITION_FAILED: Cashier role inheritance fix is missing';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'MIGRATION_ALREADY_APPLIED: 20260729090000';
    END IF;
END
$migration_guard$;
...
VALUES (
    '20260729090000',
    'g4_phase5_store_manager_pos_access_fix',
    'Forward-only POS access fix: active Store Manager assignment may operate the assigned Store POS; Cashier remains Store-scoped and Admin inheritance remains unchanged'
);
