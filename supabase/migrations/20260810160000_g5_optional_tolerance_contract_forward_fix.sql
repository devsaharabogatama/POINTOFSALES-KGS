-- G5 corrective forward fix: formalize optional Supplier Invoice tolerance.
-- The earlier function-only Phase 12 script may already be live and did not
-- write the migration ledger. This forward-only migration is authoritative.
...
BEGIN
    IF to_regclass('private.kgs_schema_migrations') IS NULL
       ...
       OR NOT EXISTS (
        ...
        RAISE EXCEPTION
            'MIGRATION_PRECONDITION_FAILED: G5 Supplier Invoice/Payment chain incomplete';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'MIGRATION_ALREADY_APPLIED: 20260810160000';
    END IF;
    ...
    IF to_regprocedure(
        ...
        RAISE EXCEPTION
            'MIGRATION_PRECONDITION_FAILED: Supplier Invoice total resolver missing';
    END IF;
END
$migration_guard$;
...
BEGIN
    ...
    ELSE
        -- Approved optional-policy behavior. Quantity allocation remains exact;
        -- only value variance becomes flexible and remains snapshotted.
...
REVOKE ALL ON FUNCTION private.refresh_supplier_invoice_totals(UUID,UUID)
FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION private.refresh_supplier_invoice_totals(UUID,UUID)
TO service_role;

INSERT INTO private.kgs_schema_migrations(version,migration_name,notes)
VALUES (
    '20260810160000',
    'g5_optional_tolerance_contract_forward_fix',
    'Forward-only formalization of optional Supplier Invoice value tolerance; exact Receipt quantity allocation and variance snapshots remain mandatory'
);
