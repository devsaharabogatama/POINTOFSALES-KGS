-- G6 phase 8E postflight. SAFETY: SELECT-only.
WITH checks(check_name,status,violation_rows,details) AS (
 ...
 FROM private.kgs_schema_migrations WHERE version='20260814150000'
 UNION ALL SELECT 'purchase_ap_controlled_queue_routines',
 ...
 WHERE namespace.nspname='public' AND routine.proname IN(
  'preview_purchase_ap_posting_queue','process_purchase_ap_posting_queue')
 UNION ALL SELECT 'purchase_ap_controlled_queue_rpc_boundary',
  CASE WHEN count(*) FILTER(WHERE has_function_privilege('anon',routine.oid,'EXECUTE'))=0
    AND count(*) FILTER(WHERE has_function_privilege('authenticated',routine.oid,'EXECUTE'))=2
 ...
 WHERE namespace.nspname='public' AND routine.proname IN(
  'preview_purchase_ap_posting_queue','process_purchase_ap_posting_queue')
 UNION ALL SELECT 'purchase_ap_queue_scope_contract',
 ...
 WHERE constraint_state.conrelid='public.finance_posting_queue_runs'::regclass
  ...
  AND pg_get_constraintdef(constraint_state.oid) LIKE '%PURCHASE_AP%'
 ...
 UNION ALL SELECT 'historical_purchase_ap_final_effect_closed',
 ...
 WHERE event.system_event_key IN('GOODS_RECEIPT','SUPPLIER_INVOICE','SUPPLIER_PAYMENT')
 UNION ALL SELECT 'purchase_ap_controlled_scope','INFO',0,jsonb_build_object(
