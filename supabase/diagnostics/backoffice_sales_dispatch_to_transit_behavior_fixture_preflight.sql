-- SELECT-only fixture inventory for Backoffice Dispatch rollback behavior.
WITH eligible AS (
  ...
  WHERE company.status='ACTIVE'
    AND EXISTS (
      ...
      WHERE category.company_id=company.id
        AND category.system_key='STOCK_TRANSFER'
  ...
  UNION ALL
  SELECT 'canonical_dispatch_company_fixture',
