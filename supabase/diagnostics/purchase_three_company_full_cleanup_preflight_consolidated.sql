-- Consolidated, SELECT-only preflight for the approved KMS/SMS/LSM full
-- procurement cleanup. One statement intentionally returns every gate in one
-- Supabase SQL Editor result grid.
...
WITH target_company(company_id,expected_name,short_code) AS (
  ...
  SELECT target.*,company.company_name,company.status company_status,
    company.timezone,setting.replenishment_mode,setting.cutoff_local_time,
  ...
  WHERE document.status='POSTED'
    AND document.company_id IN(SELECT company_id FROM target_company)
  ...
  SELECT order_line.company_id,order_line.document_id,
    ...
    count(DISTINCT invoice.id) FILTER(WHERE invoice.status='VALIDATED')
      validated_bill_count,
  ...
  SELECT document.short_code,document.order_no,receipt.receipt_no,
    ...
    allocation.quantity_base-COALESCE(prior.returned_base_qty,0)
      required_return_base_qty,
    allocation.product_batch_id,COALESCE(batch.qty_remaining,0)
      fifo_available_base_qty,
    allocation.product_batch_id IS NULL OR batch.id IS NULL
      OR batch.qty_remaining<allocation.quantity_base-
        COALESCE(prior.returned_base_qty,0) blocked
  FROM active_po document
  JOIN public.supplier_order_lines order_line
  ...
  SELECT target.short_code,document.company_id,document.id,document.request_no,
    document.status,document.master_version,count(line.id) line_count,
    COALESCE(sum(GREATEST(line.requested_base_qty-
      COALESCE(allocation.allocated_base_qty,0),0)),0) open_base_qty,
  ...
  SELECT target.short_code,run.company_id,run.business_date,run.mode_snapshot,
    run.status,run.attempt_count,run.operation_id,run.error_code,run.last_attempted_at
  FROM target_company target
  ...
  SELECT 10 sort_order,'cleanup_company_scope'::text check_name,
    CASE WHEN count(*)=3
        AND count(*) FILTER(WHERE company_name=expected_name
          AND company_status='ACTIVE')=3
        AND count(*) FILTER(WHERE replenishment_mode='AUTO_RO')=3
      THEN 'PASS' ELSE 'BLOCKER' END status,
    (3-count(*) FILTER(WHERE company_name=expected_name
      AND company_status='ACTIVE'))::bigint violation_rows,
    jsonb_build_object('companies',COALESCE(jsonb_agg(jsonb_build_object(
      ...
      'actualName',company_name,'status',company_status,'timezone',timezone,
      ...
      ORDER BY short_code),'[]'::jsonb),'requiredMode','AUTO_RO',
      'modeRule','Cleanup preserves the configured AUTO_RO flow; it does not switch Company mode') details
  FROM company_scope

  UNION ALL
  SELECT 20,'active_ro_cleanup_inventory','INFO',count(*)::bigint,
    jsonb_build_object('activeRows',count(*),
      ...
      'documents',COALESCE((SELECT jsonb_agg(to_jsonb(document)
        ...
        FROM active_ro document),'[]'::jsonb))
  FROM active_ro

  UNION ALL
  SELECT 30,'active_po_cleanup_inventory','INFO',count(*)::bigint,
    jsonb_build_object('activeRows',count(*),
      ...
      'documents',COALESCE(jsonb_agg(to_jsonb(po_inventory)
        ORDER BY short_code,order_date,order_no),'[]'::jsonb))
  FROM po_inventory

  UNION ALL
  SELECT 40,'purchase_return_fifo_availability',
    CASE WHEN count(*) FILTER(WHERE blocked)=0 THEN 'PASS' ELSE 'BLOCKER' END,
    count(*) FILTER(WHERE blocked)::bigint,
    jsonb_build_object('requiredAllocationRows',count(*),
      ...
      'blocked',COALESCE(jsonb_agg(to_jsonb(return_source)
        ...
        FILTER(WHERE blocked),'[]'::jsonb))
  FROM return_source

  UNION ALL
  SELECT 50,'active_stock_request_coverage',
    CASE WHEN COALESCE(sum(open_base_qty),0)=0 THEN 'PASS' ELSE 'BLOCKER' END,
    count(*) FILTER(WHERE open_base_qty>0)::bigint,
    jsonb_build_object(
      ...
      'documents',COALESCE(jsonb_agg(to_jsonb(active_request)
        ORDER BY short_code,request_no),'[]'::jsonb))
  FROM active_request

  UNION ALL
  SELECT 60,'today_scheduler_reuse_boundary',
    CASE WHEN count(*) FILTER(WHERE status IN('GENERATED','NO_DEMAND'))=0
      THEN 'PASS' ELSE 'BLOCKER' END,
    count(*) FILTER(WHERE status IN('GENERATED','NO_DEMAND'))::bigint,
    jsonb_build_object(
      'rule','A GENERATED/NO_DEMAND run is reused and cannot create a replacement batch',
      'runs',COALESCE(jsonb_agg(to_jsonb(today_run) ORDER BY short_code),'[]'::jsonb))
  FROM today_run

  UNION ALL
  SELECT 70,'negative_on_hand_raw_target','INFO',count(*)::bigint,
    jsonb_build_object('negativeStockRows',count(*),
      ...
      'rows',COALESCE(jsonb_agg(to_jsonb(negative_stock)
        ORDER BY short_code,product_name,warehouse_name),'[]'::jsonb))
  FROM negative_stock
)
