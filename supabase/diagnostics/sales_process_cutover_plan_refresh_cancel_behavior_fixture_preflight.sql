-- SELECT-only exact fixture gate for refresh/cancel rollback behavior.
WITH linked_actor AS (
  ...
  SELECT 'refresh_cancel_behavior_linked_actor'::text check_name,
  ...
  UNION ALL
  SELECT 'refresh_cancel_behavior_company_fixture',
    CASE WHEN count(*)>0 THEN 'PASS' ELSE 'BLOCKER' END,
    CASE WHEN count(*)>0 THEN 0 ELSE 1 END::bigint,
    jsonb_build_object('eligibleCompaniesWithoutOpenPlan',count(*),
