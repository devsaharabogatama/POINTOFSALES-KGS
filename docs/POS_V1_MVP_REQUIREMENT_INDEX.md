...
| POS-012 | Terminal Price Override | Kebijakan per Terminal default OFF; bila ON seluruh Cashier sah dapat mengubah harga final per line yang mengalahkan Pricelist, sedangkan line tanpa override tetap memakai resolver canonical; server validation, snapshot, dan audit wajib. | `POS_TERMINAL_PRICE_OVERRIDE_PLAN.md` | Post-UAT adjustment |
| POS-013 | Order Reservation & Dispatch | Konfirmasi POS membuat Sales Order dan `Reserved Out` tanpa mengurangi On Hand; Stock/FIFO/Movement final hanya saat Surat Jalan di-Dispatch. Kekurangan dihimpun per sesi, perubahan hanya menyinkronkan Draft PO dan memakai delta/amendment setelah PO confirmed. Dispatch dan verifikasi pembayaran menjadi event Finance terpisah. | `POS_ORDER_RESERVATION_DISPATCH_FINANCE_PLAN.md` | Post-pilot architecture change ODR-1—ODR-6 |
| POS-014 | Revisi Order | Order Confirmed/Reserved yang belum Dispatch dan belum mempunyai payment verified direvisi melalui Draft replacement. Source tetap aktif sampai replacement Confirm berhasil secara atomik; nomor Invoice/SJ baru dan lineage/audit wajib dipertahankan. | `SALES_ORDER_REVISION_SPEC.md` | Additive post-ODR cutover |
| PUR-001 | Purchasing | Default Company tetap Manual: RO → PO → Receive → Supplier Bill → Payment/Post. Mode opsional AUTO_RO/AUTO_PO hanya merekap On Hand negatif pukul 23:59, mengurangi open purchase agar tidak ganda, mengabaikan Reserve/Transit, mengelompokkan satu batch harian lintas-Gudang, dan tetap mewajibkan konfirmasi Receipt per Gudang. Supplier memakai priority; tanpa Supplier menjadi SUPPLIER_PENDING dan belum dapat ditagih. Seluruh channel wajib memakai satu canonical Stock/FIFO/AP/Finance Post. | `PRODUCT_STOCK_MASTERDATA_SPEC.md`, `PURCHASE_DAILY_REPLENISHMENT_SPEC.md`, `runbooks/BACKOFFICE_GOODS_RECEIPT_ROLLOUT.md` | G5 |
